...
class MainActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        val label = findViewById(R.id.ringerMode) as TextView
        label.text = "${audioManager.ringerMode}"
    }
}
...
public fun <V> Fragment.bindService(serviceName: String)
        : ReadOnlyProperty<Fragment, V> = fetchService(serviceName, Fragment::serviceBinder)
...
private fun <T, V> fetchService(serviceName: String, binder: T.(String) -> Any)
        = Lazy { t: T, property: KProperty<*> ->
                t.binder(serviceName) as V? ?: viewNotFound(serviceName, property)
            }
